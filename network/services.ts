...
export const fetchNews = (pageSize: number, page: number, sources?: string) => {
  if (sources) {
    return Http.get(
      `/everything?q=tesla&sources=${encodeURIComponent(
        sources
  ...
  return Http.get(
    `/everything?q=tsla&language=en&sortBy=publishedAt&apiKey=${process.env.NEXT_PUBLIC_NEWS_API_KEY}&page=${page}&pageSize=${pageSize}`
  );
};
